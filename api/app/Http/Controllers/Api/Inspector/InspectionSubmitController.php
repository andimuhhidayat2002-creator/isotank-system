...
class InspectionSubmitController extends Controller
{
    ...
    public function submit(Request $request, $jobId)
    {
        ...
        // Helper rule generator
        $t75Rule = function($constraints) use ($isT75, $isDraft) {
            if (!$isT75) return 'nullable'; // Completely relaxed for non-T75
            return ($isDraft ? 'nullable' : 'required') . '|' . $constraints;
        };
        ...
        $rules = [
            ...
            'surface' => $t75Rule('in:good,not_good,need_attention,na'),
            'frame' => $t75Rule('in:good,not_good,need_attention,na'),
            'tank_plate' => $t75Rule('in:good,not_good,need_attention,na'),
            'venting_pipe' => $t75Rule('in:good,not_good,need_attention,na'),
            'explosion_proof_cover' => $t75Rule('in:good,not_good,need_attention,na'),
            'grounding_system' => $t75Rule('in:good,not_good,need_attention,na'),
            'document_container' => $t75Rule('in:good,not_good,need_attention,na'),
            'safety_label' => $t75Rule('in:good,not_good,need_attention,na'),
            'valve_box_door' => $t75Rule('in:good,not_good,need_attention,na'),
            'valve_box_door_handle' => $t75Rule('in:good,not_good,need_attention,na'),
            
            // C. VALVE & PIPE SYSTEM (Legacy T75)
            'valve_condition' => $t75Rule('in:good,not_good,need_attention,na'),
            'valve_position' => $t75Rule('in:correct,incorrect'),
            'pipe_joint' => $t75Rule('in:good,not_good,need_attention,na'),
            'air_source_connection' => $t75Rule('in:good,not_good,need_attention,na'),
            'esdv' => $t75Rule('in:good,not_good,need_attention,na'),
            ...
            'pressure_gauge_condition' => $t75Rule('in:good,not_good,need_attention,na'),
            ...
            'level_gauge_condition' => $t75Rule('in:good,not_good,need_attention,na'),
            ...
            'vacuum_gauge_condition' => $t75Rule('in:good,not_good,need_attention,na'),
            'vacuum_port_suction_condition' => $t75Rule('in:good,not_good,need_attention,na'),
        ...
        try {
            ...
            // Helper to prevent "Data truncated" errors by converting empty strings to null
            $clean = function($v) {
            ...
            $logData = [
                ...
                // Use $clean() to ensure empty strings become NULL
                'surface' => $clean($validated['surface'] ?? null),
                'frame' => $clean($validated['frame'] ?? null),
                'tank_plate' => $clean($validated['tank_plate'] ?? null),
                'venting_pipe' => $clean($validated['venting_pipe'] ?? null),
                'explosion_proof_cover' => $clean($validated['explosion_proof_cover'] ?? null),
                'grounding_system' => $clean($validated['grounding_system'] ?? null),
                'document_container' => $clean($validated['document_container'] ?? null),
                'safety_label' => $clean($validated['safety_label'] ?? null),
                'valve_box_door' => $clean($validated['valve_box_door'] ?? null),
                ...
                'blind_flange' => $clean($validated['blind_flange'] ?? null),
                ...
                'ibox_temperature' => $clean($validated['temperature'] ?? null),
                ...
                'pressure_gauge_serial_number' => $clean($validated['pressure_gauge_serial'] ?? $validated['pressure_gauge_serial_number'] ?? $validated['pg_serial'] ?? $validated['pg_serial_number'] ?? $validated['pg_sn'] ?? null),
                'pressure_gauge_calibration_date' => $clean($validated['pressure_gauge_calibration_date'] ?? null),
                'pressure_gauge_valid_until' => $clean($validated['pressure_gauge_valid_until'] ?? null),
                'pressure_1' => $clean($validated['pressure_1'] ?? null),
                'pressure_1_timestamp' => $clean($validated['pressure_1_timestamp'] ?? null),
                'pressure_2' => $clean($validated['pressure_2'] ?? null),
                'pressure_2_timestamp' => $clean($validated['pressure_2_timestamp'] ?? null),
                
                'level_gauge_condition' => $clean($validated['level_gauge_condition'] ?? null),
                'level_1' => $clean($validated['level_1'] ?? null),
                'level_1_timestamp' => $clean($validated['level_1_timestamp'] ?? null),
                'level_2' => $clean($validated['level_2'] ?? null),
                'level_2_timestamp' => $clean($validated['level_2_timestamp'] ?? null),
                
                'ibox_temperature_1' => $clean($validated['ibox_temperature_1'] ?? null),
                'ibox_temperature_1_timestamp' => $clean($validated['ibox_temperature_1_timestamp'] ?? null),
                'ibox_temperature_2' => $clean($validated['ibox_temperature_2'] ?? null),
                'ibox_temperature_2_timestamp' => $clean($validated['ibox_temperature_2_timestamp'] ?? null),
                
                'vacuum_value' => $clean($validated['vacuum_value'] ?? null),
                'vacuum_unit' => $clean($validated['vacuum_unit'] ?? null),
                ...
                'psv1_calibration_date' => $clean($validated['psv1_calibration_date'] ?? null),
                'psv1_valid_until' => $clean($validated['psv1_valid_until'] ?? null),
                'psv1_status' => $clean($validated['psv1_status'] ?? null),
                'psv1_replacement_serial' => $clean($validated['psv1_replacement_serial'] ?? null),
                'psv1_replacement_calibration_date' => $clean($validated['psv1_replacement_calibration_date'] ?? null),
                
                'psv2_condition' => $clean($validated['psv2_condition'] ?? null),
                'psv2_serial_number' => $clean($validated['psv2_serial'] ?? $validated['psv2_serial_number'] ?? $validated['psv2_sn'] ?? null),
                'psv2_calibration_date' => $clean($validated['psv2_calibration_date'] ?? null),
                'psv2_valid_until' => $clean($validated['psv2_valid_until'] ?? null),
                'psv2_status' => $clean($validated['psv2_status'] ?? null),
                'psv2_replacement_serial' => $clean($validated['psv2_replacement_serial'] ?? null),
                'psv2_replacement_calibration_date' => $clean($validated['psv2_replacement_calibration_date'] ?? null),
                
                'psv3_condition' => $clean($validated['psv3_condition'] ?? null),
                'psv3_serial_number' => $clean($validated['psv3_serial'] ?? $validated['psv3_serial_number'] ?? $validated['psv3_sn'] ?? null),
                'psv3_calibration_date' => $clean($validated['psv3_calibration_date'] ?? null),
                'psv3_valid_until' => $clean($validated['psv3_valid_until'] ?? null),
                'psv3_status' => $clean($validated['psv3_status'] ?? null),
                'psv3_replacement_serial' => $clean($validated['psv3_replacement_serial'] ?? null),
                'psv3_replacement_calibration_date' => $clean($validated['psv3_replacement_calibration_date'] ?? null),
                
                'psv4_condition' => $clean($validated['psv4_condition'] ?? null),
                'psv4_serial_number' => $clean($validated['psv4_serial'] ?? $validated['psv4_serial_number'] ?? $validated['psv4_sn'] ?? null),
                'psv4_calibration_date' => $clean($validated['psv4_calibration_date'] ?? null),
                'psv4_valid_until' => $clean($validated['psv4_valid_until'] ?? null),
                'psv4_status' => $clean($validated['psv4_status'] ?? null),
                'psv4_replacement_serial' => $clean($validated['psv4_replacement_serial'] ?? null),
                'psv4_replacement_calibration_date' => $clean($validated['psv4_replacement_calibration_date'] ?? null),
